...
top-10.  Every later question-answering step searches both the original question
and an original-question-anchored Step Definer task, then exposes a stable 7/3
deduplicated union.  Aggregate tasks retrieve nothing only when grounded prior
state exists.  The Planner may emit one to five steps, QA can stop early, and the
Step Definer summarizes the completed state; this is not a fixed one-hop/two-hop
pipeline.
...
# Step 1 uses the original question. Later steps search both the original
# question and an anchored Step Definer task; fusion retains seven anchor slots
# and up to three unique task slots without increasing passage exposure.
K = 10
HOP1 = 7
ANCHOR_K = 7
